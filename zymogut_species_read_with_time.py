...
for i in range(900, 172801, 900):
	...
	for j in range(mark, len(Time)):
		...
		if Species[title] == 'Veillonella_rogosae':
			Veillonella_rogosae_length += 1
		elif Species[title] == 'Salmonella_enterica':
			Salmonella_enterica_length += 1
		elif Species[title] == 'Saccharomyces_cerevisiae':
			Saccharomyces_cerevisiae_length += 1
		elif Species[title] == 'Roseburia_hominis':
			Roseburia_hominis_length += 1
		elif Species[title] == 'Prevotella_corporis':
			Prevotella_corporis_length += 1
		elif Species[title] == 'Lactobacillus_fermentum':
			Lactobacillus_fermentum_length += 1
		elif Species[title] == 'Fusobacterium_nucleatum':
			Fusobacterium_nucleatum_length += 1
		elif Species[title] == 'Faecalibacterium_prausnitzii':
			Faecalibacterium_prausnitzii_length += 1
		elif Species[title] == 'Escherichia_coli':
			Escherichia_coli_length += 1
		elif Species[title] == 'Clostridioides_difficile':
			Clostridioides_difficile_length += 1
		elif Species[title] == 'Candida_albican':
			Candida_albican_length += 1
		elif Species[title] == 'Bifidobacterium_adolescentis':
			Bifidobacterium_adolescentis_length += 1
		elif Species[title] == 'Akkermansia_muciniphila':
			Akkermansia_muciniphila_length += 1
		elif Species[title] == 'Methanobrevibacter_smithii':
			Methanobrevibacter_smithii_length += 1
		elif Species[title] == 'Enterococcus_faecalis':
			Enterococcus_faecalis_length += 1
		elif Species[title] == 'Clostridium_perfringens':
			Clostridium_perfringens_length += 1
		elif Species[title] == 'Bacteroides_fragilis':
			Bacteroides_fragilis_length += 1
		if Time[j] > i:
			mark = j + 1
			break
	print(str(i/3600) + '\t' + str(Human_length) + '\t' + str(Veillonella_rogosae_length) + '\t' + str(Salmonella_enterica_length) + '\t' + str(Saccharomyces_cerevisiae_length) + '\t' 
		+ str(Roseburia_hominis_length) + '\t' + str(Prevotella_corporis_length) + '\t' + str(Lactobacillus_fermentum_length) + '\t' + str(Fusobacterium_nucleatum_length) + '\t' +
		 str(Faecalibacterium_prausnitzii_length) + '\t' + str(Escherichia_coli_length) + '\t' + str(Clostridioides_difficile_length) + '\t' + str(Candida_albican_length) + '\t' +
		 str(Bifidobacterium_adolescentis_length) + '\t' + str(Akkermansia_muciniphila_length) + '\t' + str(Methanobrevibacter_smithii_length) + '\t' + str(Enterococcus_faecalis_length) + '\t' 
		 + str(Clostridium_perfringens_length) + '\t' + str(Bacteroides_fragilis_length))
